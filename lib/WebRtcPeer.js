...
function insertMediaIdSlotInHtmlDom() {
  var mediaIdSlot = document.createElement('div');
  mediaIdSlot.id = 'electron-media-source-id';
  mediaIdSlot.style = 'position:absolute;opacity:0;width:0;height:0;';
  document.body.append(mediaIdSlot);
  return mediaIdSlot;
}

//insert screen-id slot for electron
var electronMediaIdSlot = insertMediaIdSlotInHtmlDom();
...
function WebRtcPeer(mode, options, callback) {
  ...
  if (mode !== 'recvonly' && !videoStream) {
    ...
    if (browser.name === 'Electron' && (sendSource === 'screen' || sendSource === 'mix')) {
      if (electronMediaIdSlot.innerText.length > 0)
        console.log('Electron: sharing window with provided sourceId: ' + electronMediaIdSlot.innerText)
